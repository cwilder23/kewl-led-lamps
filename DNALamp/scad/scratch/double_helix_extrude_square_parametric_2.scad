...
translate([0, 150, 0]) {
    ...
    difference(){       // Create helix ladder holes
        ...
        difference() {  // Hollows out helix path
            helix_extrude(angle=angle * helixRadius, height=height, $fn=precision) {
                translate([dnaRadius, 0, 0]) {
                    square(outerHelixSquare, center= true);
                }
            }
             helix_extrude(angle=angle * helixRadius + eps, height=height, $fn=precision) {
                translate([dnaRadius, 0, 0]) {
        ...
        difference() {  // Hollows out helix path
            ...
            helix_extrude(angle=360 * helixRadius + eps, height=height, $fn=precision) {
                translate([-dnaRadius, 0, 0]) {
                    square(innerHelixSquare, center= true);
                }
            }
